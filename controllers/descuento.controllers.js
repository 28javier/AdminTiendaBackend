let Descuento = require('../models/descuento.models');
var fs = require('fs');
...
const registro_descuento_admin = async function (req, res) {
    if (req.user) {
        if (req.user.rol == 'Admin-Rol') {
            ...
            res.status(200).send({ message: 'Descuento registrado correctamente', data: reg });
        } else {
...
const eliminar_descuento_admin = async (req, res) => {
    if (req.user) {
        if (req.user.rol = 'Admin-Rol') {
            ...
            res.status(200).send({ message: 'Descuento Eliminado Correctamente ', data: reg });//linea que pinta lo eliminado
        } else {
            res.status(500).send({ message: 'NoAccess' });
        }
    } else {
